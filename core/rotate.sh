...
" <<< "$CF_DOMAIN_RESPONSE" 2>/dev/null)
    ...
    for attempt in 1 2 3; do
        OLD_RECS_JSON=$(curl -s -X GET "https://api.cloudflare.com/client/v4/zones/$CF_ZONE_ID/dns_records?per_page=100" \
            -H "Authorization: Bearer $CF_API_TOKEN" -H "Content-Type: application/json")
        if [ -n "$OLD_RECS_JSON" ]; then
            break
        fi
        local sleep_time=$(( attempt + RANDOM % 2 ))
...
" <<< "$OLD_RECS_JSON" 2>/dev/null)
    ...
    local A_SUCCESS="false"
    local A_RESP=""
    for attempt in 1 2 3; do
        A_RESP=$(curl -s -X POST "https://api.cloudflare.com/client/v4/zones/$CF_ZONE_ID/dns_records" \
            -H "Authorization: Bearer $CF_API_TOKEN" \
            -H "Content-Type: application/json" \
...
" <<< "$OLD_RECS_JSON" 2>/dev/null)
        ...
        local AAAA_SUCCESS="false"
        local AAAA_RESP=""
        for attempt in 1 2 3; do
            AAAA_RESP=$(curl -s -X POST "https://api.cloudflare.com/client/v4/zones/$CF_ZONE_ID/dns_records" \
                -H "Authorization: Bearer $CF_API_TOKEN" \
                -H "Content-Type: application/json" \
